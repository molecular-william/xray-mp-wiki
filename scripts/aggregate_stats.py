        ...
        for pub in data.get("publications", []) or []:
            ...
            for p in pub.get("purifications", []) or []:
                ...
                for step in p.get("steps", []):
                    ...
                    for det in extract_detergent(step):
                        slug = det[0]
                        if slug and (not detergents_seen or detergents_seen[-1] != slug):
                            detergents_seen.append(slug)
        if len(set(detergents_seen)) > 1:
            exchanges.append((title, fam, detergents_seen))
            key = " → ".join(detergents_seen[:3])
