---
title: "Major Facilitator Superfamily (MFS)"
created: 2026-07-06
updated: 2026-07-06
type: concept
category: concepts
layout: default
tags: [concept-protein-family, subdirectory-protein-families]
sources: [doi/10.1016##j.jbc.2021.100479, doi/10.1016##j.jmb.2018.02.026, doi/10.1016##j.str.2014.06.008, doi/10.1016##j.str.2014.12.012, doi/10.1038##EMBOJ.2010.309, doi/10.1038##nature03692, doi/10.1038##nature13306, doi/10.1038##nature14655, doi/10.1038##ncomms5521, doi/10.1038##ncomms8097, doi/10.1038##ncomms8177, doi/10.1038##s41467-023-42073-0, doi/10.1038##s41594-020-0425-5, doi/10.1126##science.1087619, doi/10.1126##science.1099314, doi/10.1126##science.1100717, doi/10.1073##pnas.0709388104, doi/10.26508##lsa.202000858, doi/10.7554##eLife.34995, doi/10.1016##j.cell.2016.10.004]
verified: none
---

# Major Facilitator Superfamily (MFS)

## Overview
The Major Facilitator Superfamily (MFS) is one of the largest and most
diverse families of secondary active membrane transporters, found across all
kingdoms of life. MFS proteins transport a wide range of substrates
including sugars, drugs, metabolites, ions, peptides, and lipids. They
typically consist of 12 transmembrane helices arranged in two pseudosymmetrical
N-terminal and C-terminal domains, each containing a 6-helix bundle. Transport
occurs via an alternating access mechanism, often coupled to proton or sodium
gradients.


## Mechanism/Details
The MFS includes numerous subfamilies classified by transport mechanism and
substrate specificity. Major subfamilies include the sugar porter (SP) family,
the drug:H+ antiporter (DHA) families, the oligosaccharide:H+ symporter (OHS)
family, the proton-dependent oligopeptide transporter (POT) family, and the
nucleoside:H+ symporter (NHS) family. The first high-resolution MFS structure
was GlpT from E. coli (PDB 1PW4, 3.3 Å), which revealed the characteristic
12-helix pseudo-twofold architecture.

MFS transporters have been extensively studied in the context of multidrug
resistance, as several members (e.g., EmrD, MdfA) export clinically relevant
antibiotics. More recently, MFS flippases such as LtaA from S. aureus
(PDB 6S7V) have been shown to transport lipid substrates, expanding the known
functional repertoire of the family.


## Examples in Membrane Protein Work
- [GlpT](/xray-mp-wiki/proteins/mfs-transporters/glpT/) — E. coli glycerol-3-phosphate/Pi antiporter; first MFS structure (PDB 1PW4, 3.3 Å). Revealed the 12-helix pseudo-twofold architecture and rocker-switch mechanism.
- [LacY](/xray-mp-wiki/proteins/lacy/) — E. coli lactose permease; extensively characterized proton-coupled symporter with inward-open (PDB 2CFP) and outward-open (PDB 2Y5Y) structures.
- [NhaA](/xray-mp-wiki/proteins/slc-transporters/nhaa/) — E. coli Na+/H+ antiporter; unusual 10-TM MFS fold (PDB 1YQY, 3.45 Å), first structure of a Na+/H+ antiporter.
- [LtaA](/xray-mp-wiki/proteins/mfs-transporters/ltaa/) — S. aureus lipid-linked disaccharide flippase (PDB 6S7V, 3.3 Å). Demonstrates MFS fold adapted for amphiphilic lipid substrates.
- [Rv1410 MFS Transporter (P55)](/xray-mp-wiki/proteins/mfs-transporters/rv1410/) — TAG:H+ antiporter from M. tuberculosis, DHA2 subclass MFS member with conserved E157-R426 ion lock.

## Related Concepts


## Cross-References
- [GlpT (Glycerol-3-Phosphate/Pi Antiporter)](/xray-mp-wiki/proteins/mfs-transporters/glpT/) — First MFS protein crystallized, revealing the canonical 12-helix fold
- [LacY (Lactose Permease)](/xray-mp-wiki/proteins/lacy/) — Best-characterized MFS symporter with structures in multiple states
- [NhaA (Na+/H+ Antiporter)](/xray-mp-wiki/proteins/slc-transporters/nhaa/) — Atypical 10-TM MFS member, first Na+/H+ antiporter structure
- [LtaA (S. aureus Lipid-Linked Disaccharide Flippase)](/xray-mp-wiki/proteins/mfs-transporters/ltaa/) — MFS flippase demonstrating transport of lipid substrates
- [Rv1410 MFS Transporter (P55)](/xray-mp-wiki/proteins/mfs-transporters/rv1410/) — M. tuberculosis MFS drug resistance transporter
- [Alternating Access Mechanism](/xray-mp-wiki/concepts/transport-mechanisms/alternating-access-mechanism/) — Core transport mechanism of MFS transporters
- [Rocker-Switch Mechanism](/xray-mp-wiki/concepts/structural-mechanisms/rocker-switch-mechanism/) — MFS-specific variant of alternating access
- [POT Family](/xray-mp-wiki/concepts/transport-mechanisms/pot-family/) — Proton-dependent oligopeptide transporter family within MFS
