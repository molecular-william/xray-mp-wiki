---
title: Fab21 Antibody Fragment
created: 2026-06-02
updated: 2026-06-02
type: reagent
category: reagents
layout: default
tags: [antibody, subdirectory-antibodies]
sources: [doi/10.1038##nature08201]
verified: false
---

# Fab21 Antibody Fragment

## Overview

Fab21 is a monoclonal antibody fragment (Fab) derived from a type-2a IgG raised against Escherichia coli AdiC. It was used to form a stable complex with Salmonella AdiC for X-ray crystallography, enabling phasing and structure determination at 3.2 A resolution. The Fab fragment binds near the intracellular subunit interface of the AdiC homodimer, straddling the two-fold axis. Approximately 25 monoclonal antibodies were tested for crystallization to obtain Fab21, which was selected based on its ability to form stable complexes and yield well-diffracting crystals.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [AdiC Arginine/Agmatine Antiporter](/xray-mp-wiki/proteins/slc-transporters/adic/) | Stoichiometric excess over AdiC | Fab21 was raised by inoculating mice with E. coli AdiC and screening ELISA-positive hybridomas for stable complexes by size-exclusion chromatography, crystallization, and diffraction quality. The Fab21-AdiC complex was purified on Superdex 200 and crystallized by hanging-drop vapor diffusion. Crystals diffracted to 3.2 A resolution.
 | Enabled structure determination of AdiC at 3.2 A resolution (P1 space group) via anomalous dispersion phasing from SeMet derivatives, with NCS averaging and solvent flattening.
 |

## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References
