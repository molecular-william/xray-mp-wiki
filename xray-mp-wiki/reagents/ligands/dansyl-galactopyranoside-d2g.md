---
title: Dansyl-galactopyranoside (D2G)
created: 2026-06-04
updated: 2026-06-04
type: reagent
category: reagents
layout: default
tags: [additive-ligand, subdirectory-ligands]
sources: [doi/10.1038##ncomms4009]
verified: false
---

# Dansyl-galactopyranoside (D2G)

## Overview

Dansyl-galactopyranoside (D2G), formally 2'-(N-dansyl)aminoalkyl-1-thio-beta-D-galactopyranoside, is a fluorescent galactopyranoside analog used as a probe for studying sugar binding to melibiose permease (MelB). D2G binds specifically to MelB and is used in fluorescence resonance energy transfer (FRET) assays and steady-state melibiose accumulation assays to measure cation-stimulated sugar binding. Melibiose can displace D2G from MelB, allowing determination of inhibition constants (IC50).


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [Melibiose Permease from Salmonella typhimurium (MelBSt)](/xray-mp-wiki/proteins/mfs-transporters/melbst/) | 10 uM | Trp->D2G FRET assay with RSO vesicles; excitation at 290 nm, emission at 490 nm | D2G binding detected by FRET; displacement by 120 mM melibiose; stimulation by 20 mM Na+ or Li+ |

## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References
