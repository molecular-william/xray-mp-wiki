---
title: "Prostaglandin D2 (PGD2)"
created: 2026-06-11
updated: 2026-06-11
type: reagent
category: reagents
layout: default
tags: [ligand, subdirectory-ligands]
sources: [doi/10.1073##pnas.2102813118]
verified: regex
---

# Prostaglandin D2 (PGD2)

## Overview

Prostaglandin D2 (PGD2) is an endogenous eicosanoid signaling lipid derived from arachidonic acid. It is the primary endogenous ligand for two G-protein-coupled receptors: DP1 (PTGDR1) and DP2/CRTH2 (PTGDR2). PGD2 is a major mediator of inflammatory responses, particularly in type 2 inflammation, asthma, and allergic reactions. [Human CRTH2 (PGD2 Receptor)](/xray-mp-wiki/proteins/gpcr/crth2/) signaling through PGD2 induces chemotaxis in Th2 cells, eosinophils, and basophils.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

No examples from this wiki.
## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [CRTH2 (DP2)](/xray-mp-wiki/proteins/gpcr/crth2/) — Primary pro-inflammatory PGD2 receptor; structure solved with 15mPGD2 derivative (PDB 7M8W)
- [15R-methyl-PGD2 (15mPGD2)](/xray-mp-wiki/reagents/ligands/15r-methyl-pgd2/) — Synthetic derivative with higher affinity for CRTH2, used for structure determination
- [Prostanoid Receptor Family](/xray-mp-wiki/concepts/signaling-receptors/prostanoid-receptor-family/) — PGD2 acts on DP1 and DP2/CRTH2 members of the prostanoid receptor family
