---
title: "S1RA (4-[2-(5-methyl-1-(naphthalene-2-yl)-1H-pyrazol-3-yl)oxy]ethyl]morpholine)"
created: 2026-06-08
updated: 2026-06-08
type: reagent
category: reagents
layout: default
tags: [additive-ligand, ligand]
sources: [doi/10.1038##s41467-022-28946-w]
verified: regex
---

# S1RA (4-[2-(5-methyl-1-(naphthalene-2-yl)-1H-pyrazol-3-yl)oxy]ethyl]morpholine)

## Overview

S1RA is a sigma-1 receptor ligand purchased from Topscience Co. Ltd. It was used in crystallization and soaking experiments with the Xenopus laevis sigma-1 receptor (xlS1R). S1RA was soaked into pre-formed xlS1R crystals and also used with the xlS1R C179/C203 double mutant to validate the PATH2 ligand entry hypothesis.


## Properties


## Use in Membrane Protein Work

### Sigma-1 receptor crystallization

Soaked into xlS1R crystals to obtain closed conformation structure (PDB 7W2D) and into xlS1R C179/C203 mutant crystals (PDB 7W2H)


### Ligand entry pathway validation

In the xlS1R C179/C203-S1RA structure, an additional S1RA molecule was observed near the ligand entrance between alpha4 and alpha5, possibly representing an entering or exiting pose for the ligand


## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [Sigma-1 Receptor from Xenopus laevis (xlS1R)](/xray-mp-wiki/proteins/receptors-signaling/xl-sigma-1-receptor/) | Not specified | Soaked into xlS1R C179/C203 mutant crystals after growth with [PRE084](/xray-mp-wiki/reagents/additives/pre084/) | S1RA bound in the ligand-binding pocket; additional density observed near the alpha4/alpha5 entrance |

## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [Sigma-1 Receptor from Xenopus laevis (xlS1R)](/xray-mp-wiki/proteins/receptors-signaling/xl-sigma-1-receptor/) — Related protein structure
- [PRE084](/xray-mp-wiki/reagents/additives/pre084/) — Additive used in purification or crystallization buffers
