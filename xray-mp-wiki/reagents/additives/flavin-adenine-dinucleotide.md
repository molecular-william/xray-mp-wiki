---
title: "Flavin Adenine Dinucleotide (FAD)"
created: 2026-06-08
updated: 2026-06-08
type: reagent
category: reagents
layout: default
tags: [additive-ligand, subdirectory-additives]
sources: [doi/10.1073##pnas.0904165106]
verified: regex
---

# Flavin Adenine Dinucleotide (FAD)

## Overview

Flavin adenine dinucleotide (FAD) is a redox cofactor derived from [Riboflavin (Vitamin B2)](/xray-mp-wiki/reagents/cofactors/riboflavin/). In Aquifex aeolicus SQR, FAD is covalently linked to the polypeptide via a labile persulfide bridge between its 8-methyl group and Cys-124, an unusual mode of FAD attachment.


## Properties

- **Chemical formula**: C27H33N9O15P2
- **Molecular weight**: 785.55
- **Class**: flavin cofactor

## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

No examples from this wiki.
## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [Sulfide:Quinone Oxidoreductase (SQR)](/xray-mp-wiki/proteins/enzymes/sulfide-quinone-oxidoreductase/) — FAD is the redox cofactor of SQR
- [Riboflavin (Vitamin B2)](/xray-mp-wiki/reagents/cofactors/riboflavin/) — FAD is derived from riboflavin
- [Riboflavin (Vitamin B2)](/xray-mp-wiki/reagents/cofactors/riboflavin/) — Related ligand or cofactor
