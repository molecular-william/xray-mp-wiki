---
title: Potassium Ferricyanide
created: 2026-06-11
updated: 2026-06-11
type: reagent
category: reagents
layout: default
tags: [additive-ligand, subdirectory-additives]
sources: [doi/10.1038##nature21400]
verified: false
---

# Potassium Ferricyanide

## Overview

Potassium ferricyanide (K3[Fe(CN)6]) is an oxidizing agent used as an artificial electron acceptor in time-resolved structural studies of photosystem II. It accepts electrons from the reducing side of PSII, facilitating the turnover of the oxygen-evolving complex during flash illumination experiments.


## Properties

- **Chemical name**: Potassium ferricyanide
- **Class**: oxidizing agent / electron acceptor

## Use in Membrane Protein Work

### Artificial electron acceptor for pre-flashing PSII samples

In TR-SFX experiments on Photosystem II from Thermosynechococcus vulcanus at SACLA XFEL, potassium ferricyanide was added to the mother liquid as an electron acceptor for pre-flashed samples (10 mM) and non-pre-flashed samples (2 mM). It accepts electrons from the QB site, enabling controlled S-state transitions upon laser flash illumination.


## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [Photosystem II](/xray-mp-wiki/proteins/photosynthesis/photosystem-ii/) | 10 mM (pre-flashed), 2 mM (non-pre-flashed) | Mother liquid component for TR-SFX experiments at SACLA XFEL | Enabled controlled S-state transitions for time-resolved structural analysis |

## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [Photosystem II](/xray-mp-wiki/proteins/photosynthesis/photosystem-ii/) — Potassium ferricyanide used as electron acceptor in TR-SFX experiments
- [Time-Resolved Serial Femtosecond Crystallography (TR-SFX)](/xray-mp-wiki/methods/structure-determination/time-resolved-serial-femtosecond-crystallography/) — TR-SFX technique used with potassium ferricyanide electron acceptor
