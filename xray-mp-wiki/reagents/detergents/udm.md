---
title: "n-Undecyl-beta-D-maltoside (UDM)"
created: 2026-06-04
updated: 2026-07-09
type: reagent
category: reagents
layout: default
tags: [detergent-nonionic, subdirectory-detergents]
sources: [doi/10.1038##NATURE06717, doi/10.1038##ncomms4009, doi/10.1126##science.1110064, doi/10.1038##nature07869]
verified: none
---

# n-Undecyl-beta-D-maltoside (UDM)

## Overview

n-Undecyl-beta-D-maltoside ([UDM](/xray-mp-wiki/reagents/detergents/udm/)) is a non-ionic maltoside detergent with an undecyl (C11) hydrocarbon tail and a [Maltose](/xray-mp-wiki/reagents/additives/maltose/) headgroup. It is slightly shorter than the more common n-dodecyl-beta-D-maltoside ([DDM](/xray-mp-wiki/reagents/detergents/ddm/), C12). [UDM](/xray-mp-wiki/reagents/detergents/udm/) was used together with [DDM](/xray-mp-wiki/reagents/detergents/ddm/) in the crystallization of the E. hirae V-ATPase K ring (NtpK10 ring). The [Maltose](/xray-mp-wiki/reagents/additives/maltose/) ring was resolved for [UDM](/xray-mp-wiki/reagents/detergents/udm/) molecules at crystal contact sites, while the undecyl moieties were visible as elongated features but the [Maltose](/xray-mp-wiki/reagents/additives/maltose/) moieties at other sites were disordered.


## Properties

- **Chemical name**: n-Undecyl-beta-D-maltopyranoside
- **Molecular weight**: ~496.6 g/mol
- **Class**: detergent-nonionic

## Use in Membrane Protein Work

### Membrane protein solubilization and purification

> UDM is used for membrane extraction of MFS transporters such as MelBSt at 1.5% (w/v), followed by maintenance at lower concentrations (0.035%) during cobalt-affinity chromatography. The maltoside headgroup provides mild solubilization suitable for maintaining transporter activity and stability during purification.


### Crystallization additive for membrane proteins

> UDM was used together with DDM in the crystallization of the E. hirae V-ATPase K ring (NtpK10 ring). UDM molecules were resolved at crystal contact sites, with the maltose ring visible in electron density maps, while the undecyl moieties appeared as elongated features at some sites and were disordered at others.


## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [Melibiose Permease from Salmonella typhimurium (MelBSt)](/xray-mp-wiki/proteins/mfs-transporters/melbst/) | 1.5% (solubilization), 0.035% (chromatography buffer) | Membrane extraction at 14 mg/ml protein; column buffer during cobalt-affinity chromatography | Effective solubilization of MelBSt from E. coli membranes; maintained protein stability during purification |

## Advantages and Disadvantages

### Advantages

- Mild non-ionic maltoside detergent suitable for membrane protein purification and crystallization
- Slightly shorter alkyl chain (C11) than DDM (C12), providing different solubilization properties for hydrophobic matching
- Effective for solubilization of MFS transporters at moderate detergent concentrations

### Disadvantages

- Less commonly used than DDM (C12) — fewer characterization data available
- C11 chain may not be optimal for all membrane protein targets
- Higher CMC than DDM, requiring higher concentrations for solubilization

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [V-Type Na+-ATPase Rotor Ring (NtpK10) from Enterococcus hirae](/xray-mp-wiki/proteins/pumps-atpases/ehirae-v-atpase-k-ring/) — Used in crystallization of the NtpK10 ring
- [n-Dodecyl-beta-D-maltoside (DDM)](/xray-mp-wiki/reagents/detergents/ddm/) — Related maltoside detergent with one additional carbon in the alkyl tail
- [Maltose](/xray-mp-wiki/reagents/additives/maltose/) — Additive used in purification or crystallization buffers
- [DDM](/xray-mp-wiki/reagents/detergents/ddm/) — Detergent used in purification or crystallization
- [UDM](/xray-mp-wiki/reagents/detergents/udm/) — Detergent used in purification or crystallization
