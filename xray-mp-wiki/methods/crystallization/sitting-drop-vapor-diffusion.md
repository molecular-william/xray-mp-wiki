---
title: Sitting-Drop Vapor Diffusion
created: 2026-05-29
updated: 2026-05-29
type: method
category: methods
layout: default
tags: [crystallization-sitting-drop, crystallization-vapor-diffusion, subdirectory-crystallization]
sources: [doi/10.1128##jb.187.19.6804-6815.2005, doi/10.1016##j.jmb.2007.04.082, doi/10.1074##jbc.M113.534628, doi/10.1074##jbc.M114.622688, doi/10.1073##pnas.240224997, doi/10.1073##pnas.2215072120, doi/10.1128##mBio.03277-19, doi/10.1038##NATURE06717, doi/10.1038##ncomms3465, doi/10.1038##NSMB.1933, doi/10.1038##nature04508, doi/10.1038##ncomms8947, doi/10.15252##embj.201593285, doi/10.1093##mp##ssu005]
verified: false
---

# Sitting-Drop Vapor Diffusion

## Overview

Sitting-drop vapor diffusion is a widely used protein crystallization technique where a small drop containing protein and precipitant solution is placed on a flat platform (sitting well) above a larger reservoir of precipitant solution. As water vapor diffuses from the drop to the reservoir, the drop gradually concentrates, eventually reaching supersaturation and nucleation. This method is one of the most common approaches for membrane and soluble protein crystallization.

## Protocol

### Reagents and Materials

- {'protein_sample': 'Protein solution at appropriate concentration (typically 2-20 mg/mL)', 'reservoir': 'Precipitant solution (varies widely depending on target; common precipitants include PEGs, salts, and organic solvents)'}
- {'cryoprotectant': 'Optional cryoprotectant added to reservoir (e.g., glycerol, ethylene glycol, or high concentration of precipitant component)'}

### Steps

1. {'step': 'Drop preparation', 'description': 'Mix protein solution with reservoir solution in a desired ratio (typically 1:1, 2:1, or 1:2) on the sitting well platform. Typical drop volumes are 0.1-1 uL total.'}
2. {'step': 'Plate sealing', 'description': 'Seal the crystallization plate with a transparent adhesive film or lid to control evaporation rate. Proper sealing prevents contamination and ensures reproducible equilibration.'}
3. {'step': 'Equilibration', 'description': 'Allow the drop to equilibrate against the reservoir over hours to weeks. Water vapor diffuses from the drop to the reservoir, concentrating the protein until crystallization occurs.'}
4. {'step': 'Harvesting', 'description': 'When crystals appear, harvest them by looping and transferring to cryoprotectant solution before flash-cooling in liquid nitrogen.'}


## Advantages

- Compatible with automated 96-well and 384-well plate formats
- Simple setup with minimal equipment
- Fine control over equilibration rate
- Widely used with extensive optimization protocols
- Ideal for high-throughput screening

## Disadvantages

- Slow equilibration can delay crystal appearance
- Drop volume limited by well geometry
- Evaporation rate depends on seal quality and temperature stability
- Limited sample recovery compared to hanging-drop method

## Related Methods

- [Hanging-Drop Vapor Diffusion](/xray-mp-wiki/methods/crystallization/hanging-drop-vapor-diffusion/) — Related vapor diffusion method with drop hanging from coverslip instead of sitting on platform
- [Lipidic Cubic Phase Crystallization](/xray-mp-wiki/methods/crystallization/lipidic-cubic-phase/) — Alternative crystallization method, especially for membrane proteins

## Related Reagents

- [Polyethylene Glycol (PEG)](/xray-mp-wiki/reagents/additives/peg/) — Common precipitant used in crystallization reservoir solutions
- [Tris (Tris-HCl Buffer)](/xray-mp-wiki/reagents/buffers/tris/) — Common buffer used in crystallization solutions at various pH values
- [PEG 400 (Polyethylene Glycol 400)](/xray-mp-wiki/reagents/additives/peg-400/) — Low molecular weight PEG used as crystallization precipitant
- [PEG 4000 (Polyethylene Glycol 4000)](/xray-mp-wiki/reagents/additives/peg-4000/) — Higher molecular weight PEG used as crystallization precipitant for membrane proteins
