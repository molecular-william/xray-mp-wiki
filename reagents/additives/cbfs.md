---
title: CBFS (8-(chloromercuri)-2-dibenzofuransulfonic acid)
created: 2026-05-28
updated: 2026-05-28
type: reagent
category: reagents
layout: default
tags: [additive-ligand, subdirectory-additives]
sources: [doi/10.1016##j.str.2012.08.009]
verified: false
---

# CBFS (8-(chloromercuri)-2-dibenzofuransulfonic acid)

## Overview

CBFS (8-(chloromercuri)-2-dibenzofuransulfonic acid) is a mercurial compound used for site-directed chemical labeling of cysteine residues in proteins. It has been employed to mimic alcohol and anesthetic binding to specific residues in pentameric ligand-gated ion channels (pLGICs), providing functional validation of ligand-binding sites identified through structural studies.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

No examples from this wiki.
## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [GLIC](/xray-mp-wiki/proteins/glic/) — Used to covalently label N152C mutant, mimicking ketamine binding and validating the anesthetic binding site
