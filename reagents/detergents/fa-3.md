---
title: "FA-3 (Facade-EM Facial Amphiphile)"
created: 2026-06-11
updated: 2026-06-23
type: reagent
category: reagents
layout: default
tags: [detergent-nonionic, solubilization-detergent, additive-stabilizer, subdirectory-detergents]
sources: [doi/10.1016##j.str.2019.04.007]
verified: false
---

# FA-3 (Facade-EM Facial Amphiphile)

## Overview

FA-3 (Facade-EM, 3alpha-hydroxy-7alpha,12alpha-di-((O-beta-D-maltosyl)-2-hydroxyethoxy)-cholane) is a facial amphiphile designed for membrane protein stabilization and crystallization. Unlike conventional detergents that form micelles, facial amphiphiles present a hydrophobic face and a hydrophilic face, providing a more membrane-like environment. FA-3 was essential for obtaining well-diffracting crystals of MsbA at 2.8 A resolution and maintained MsbA ATPase activity comparable to lipid bilayer conditions (~200 nmol/min/mg). It provided superior thermal stability (Tm ~57 C) compared to conventional detergents like UDM or LMNG.


## Properties

- **Chemical name**: 3alpha-hydroxy-7alpha,12alpha-di-((O-beta-D-maltosyl)-2-hydroxyethoxy)-cholane
- **Molecular weight**: "~1250 g/mol"
- **Class**: Facial amphiphile (bile acid-derived)

## Use in Membrane Protein Work

### Membrane protein crystallization

FA-3 was used at 0.04 wt% (0.36 mM) for MsbA crystallization. The detergent was exchanged from UDM to FA-3 using centrifugal filters. Co-crystallization with lipid A and FA-3 enabled the 2.8 A structure of MsbA in an inward-facing conformation. FA-3 lacks the dissolving power of phospholipids for lipid A, which may help retain lipid A at binding sites in the crystal.


## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [MsbA Lipid A Flippase](/xray-mp-wiki/proteins/abc-transporters/msba/) |  | FA-3 used for MsbA solubilization, stabilization, and crystallization; enabled 2.8 A structure with bound lipid A (PDB 6BL6) |  |

## Advantages and Disadvantages

### Advantages

- Superior thermal stability compared to conventional detergents — MsbA in FA-3 has a Tm of ~57°C vs UDM and LMNG (Figure 2E)
- Maintains ATPase activity comparable to lipid bilayer conditions — MsbA in FA-3 shows ~200 nmol/min/mg activity, similar to MSP1D1 nanodiscs (Figure 2B)
- Facial amphiphile architecture provides a more membrane-like environment than conventional micelle-forming detergents
- Lacks the strong dissolving power of phospholipids for lipid A, helping retain native lipid substrates at binding sites during crystallization

### Disadvantages

- Complex multi-step synthesis as a bile acid-derived facial amphiphile makes it more expensive and less accessible than conventional detergents
- Limited solubilizing power for some membrane proteins compared to conventional detergents like DDM
- Weak dissolving power for lipids means it may not be suitable for extracting membrane proteins directly from native membranes without pre-solubilization

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [MsbA Lipid A Flippase](/xray-mp-wiki/proteins/abc-transporters/msba/) — FA-3 was the essential detergent for MsbA crystallization at 2.8 A resolution
