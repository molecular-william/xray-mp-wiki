---
title: Nb.AT110i1 Synthetic Nanobody
created: 2026-05-27
updated: 2026-05-27
type: reagent
category: reagents
layout: default
tags: [antibody, subdirectory-antibodies]
sources: [doi/10.1016##j.cell.2018.12.006]
verified: false
---

# Nb.AT110i1 Synthetic Nanobody

## Overview

Nb.AT110i1 is a synthetic conformation-specific nanobody discovered using a yeast-displayed synthetic nanobody library. It was affinity-matured from the parent clone Nb.AT110 and binds with high specificity to the active-state conformation of the human angiotensin II type 1 receptor (AT1R). The nanobody inserts its unusually long CDR3 loop into the receptor's intracellular transducer-binding pocket, stabilizing the activated conformation and enabling the 2.9 A resolution crystal structure of active-state AT1R bound to the peptide S1I8. Four affinity-maturing mutations (A31V, N58D, I98V, Y113N) were introduced to create Nb.AT110i1 from Nb.AT110.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| [Angiotensin II Type 1 Receptor (AT1R)](/xray-mp-wiki/proteins/at1r/) | 30 uM complex with AT1R-S1I8 | Crystallization of active-state AT1R in lipidic cubic phase | Stabilized active-state AT1R for X-ray crystallography; PDB 6DO1 at 2.9 A resolution |

## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [Angiotensin II Type 1 Receptor](/xray-mp-wiki/proteins/at1r/) — Primary target stabilized by Nb.AT110i1 in active conformation
- [Angiotensin II](/xray-mp-wiki/reagents/ligands/angiotensin-ii/) — Endogenous agonist used to select conformation-specific nanobodies
- [S1I8](/xray-mp-wiki/reagents/ligands/s1i8/) — Partial agonist peptide used in Nb.AT110i1-AT1R crystallization complex
- [Nanobody](/xray-mp-wiki/reagents/protein-tags/nanobody/) — Nb.AT110i1 is a synthetic nanobody derived from camelid VHH scaffold
- [Lipidic Cubic Phase Crystallization](/xray-mp-wiki/methods/crystallization/lipidic-cubic-phase/) — Method used to crystallize AT1R-Nb.AT110i1-S1I8 complex
