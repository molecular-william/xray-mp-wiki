---
title: BeF3 (Beryllium Fluoride)
created: 2026-06-02
updated: 2026-06-02
type: reagent
category: reagents
layout: default
tags: [ligand, subdirectory-ligands]
sources: [doi/10.1038##NSMB.2721, doi/10.1038##nature06418]
verified: false
---

# BeF3 (Beryllium Fluoride)

## Overview

Beryllium fluoride (BeF3-) is a phosphate analog used in structural biology to trap phosphorylated intermediate states of ATP-dependent enzymes. It mimics the geometry and charge distribution of the terminal phosphate group in ATP or the aspartyl-phosphate intermediate of P-type ATPases. When combined with Mg2+, BeF3- binds to the phosphorylation site and forms a tetrahedral complex that structurally resembles the transition state of phosphate transfer.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| LpCopA (Copper-Transporting P-type ATPase from Legionella pneumophila) | -- | Trapped E2P state of LpCopA for X-ray crystallography; BeF3- binds to Asp426 with Mg2+ | Structure determined at 2.0 A resolution (PDB 3B9B) |
| SERCA1a (Sarcoplasmic Reticulum Ca2+-ATPase 1a) | -- | Mimics E2P state in SERCA1a for structural comparison with LpCopA | Structural comparison reveals similar cytoplasmic domain arrangement in E2P state |
| SERCA1a (Sarcoplasmic Reticulum Ca2+-ATPase 1a) | -- | Supplementary data from doi/10.1038/nature06418; E2P-BeF3- structure in space group P212121 at 2.65 A resolution | Additional E2P structure collected at ESRF ID23-2 with wavelength 0.873 A |

## Advantages and Disadvantages

### Advantages

- Forms a stable tetrahedral complex with Mg2+ that closely mimics the aspartyl-phosphate intermediate
- Enables trapping of phosphorylated states for high-resolution structural studies
- Useful for studying the conformational changes associated with phosphorylation in ATPases

### Disadvantages

- Beryllium compounds are toxic and require careful handling
- BeF3- complexes are labile and must be prepared immediately before use
- Only traps specific phosphorylated states, not the complete reaction cycle

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [AlF4 (Aluminum Fluoride)](/xray-mp-wiki/reagents/ligands/alf4/) — Another metal fluoride phosphate analog used for trapping phosphorylated states
- [LpCopA (Copper-Transporting P-type ATPase from Legionella pneumophila)](/xray-mp-wiki/proteins/lp-copa/) — Used to trap E2P state of LpCopA for crystallography
- [Phosphoenzyme E2P State](/xray-mp-wiki/concepts/phosphoenzyme-e2p-state/) — BeF3- is used to structurally mimic the E2P intermediate state
