---
title: 11-cis-Retinal
created: 2026-05-07
updated: 2026-05-07
type: reagent
category: reagents
layout: default
tags: [ligand, chromophore, visual-pigment, gpcr-ligand]
sources: [doi/10.1016##J.JMB.2004.08.090, doi/10.1016##J.JMB.2007.03.007]
---

# 11-cis-Retinal

## Overview

11-cis-retinal is the light-sensitive chromophore covalently bound to the apoprotein opsin
to form rhodopsin, the photoreceptor GPCR in vertebrate retinal rod cells. It is linked to
Lys296 on helix 7 via a protonated Schiff base. Absorption of a photon at ~500 nm
photoisomerises the retinal to all-trans form within picoseconds, initiating the visual
transduction cascade. In the dark state, 11-cis-retinal acts as an inverse agonist, keeping
the basal activity of rhodopsin extremely low. The chromophore is bound in a tight pocket
formed by seven transmembrane helices, with extensive van der Waals contacts and hydrogen
bonding networks stabilizing its conformation.


## Properties


## Use in Membrane Protein Work

No specific use cases documented.
## Examples from This Wiki

| Protein | Concentration | Context | Result |
|---|---|---|---|
| Bovine Rhodopsin (Dark State) | 1:1 stoichiometry | Covalently bound to Lys296 via protonated Schiff base in the dark state | Inverse agonist effect keeps basal activity below that of ligand-free opsin |
| Rhodopsin N2C/D282C Mutant | 1:1 stoichiometry | Covalently bound to Lys296; expressed in COS mammalian cells with exogenous 11-cis-retinal supplied before solubilization | Stabilized holoprotein form suitable for crystallization |

## Binding Mode

### Binding to Opsin (rhodopsin apoprotein)


## Advantages and Disadvantages

No advantages/disadvantages recorded.

## Comparison with Related Reagents

No comparison data available.

## Cross-References

- [Bovine Rhodopsin (Dark State)](/xray-mp-wiki/proteins/bovine-rhodopsin/) — Primary chromophore binding partner; defines the visual pigment
- [Rhodopsin N2C/D282C Mutant](/xray-mp-wiki/proteins/rhodopsin-n2c-d282c-mutant/) — Recombinant rhodopsin mutant requiring 11-cis-retinal for holoprotein formation
