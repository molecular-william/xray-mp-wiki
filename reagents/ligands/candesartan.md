---


title: Candesartan
created: 2026-04-27
updated: 2026-04-27
type: reagent
tags: [additive-ligand]
sources: [doi/10.1016##j.cell.2015.04.011]


category: reagents
---
layout: default



# Candesartan

## Chemical Properties

- **Class**: Angiotensin receptor blocker (ARB), non-peptide antagonist
- **Scaffold**: Biphenyl-tetrazole with benz[imidazole](//xray-mp-wiki/reagents/additives/imidazole/) carboxylic acid moiety
- **Relation**: ZD7155 is a precursor to candesartan
- **Pharmacology**: Insurmountable inverse agonist with slow dissociation rate

## Binding to AT1R

Candesartan binds to the ligand-binding pocket of the human angiotensin II type 1 receptor (AT1R). Based on docking studies into the 2.9 Å SFX/XFEL structure (PDB: 4YAY):

- **Tetrazole moiety**: Forms a salt bridge with Arg167ᴱᴰᴸ² (ECL2)
- **Benzimidazole carboxylic acid**: Forms a **second salt bridge** with Arg167ᴱᴰᴸ²
- **Lys199⁵·⁴²**: Forms an **additional salt bridge** with the tetrazole moiety — this extra interaction stabilizes candesartan binding
- **Triple interaction network**: Arg167ᴱᴰᴸ² (2 salt bridges) + Lys199⁵·⁴² (1 salt bridge) = highest affinity among ARBs

## Clinical Properties

- **Insurmountable inverse agonist**: Cannot be overcome by high AngII [amicon-filters](//xray-mp-wiki/methods/purification/amicon-filters/)s
- **Slow dissociation rate**: Long receptor occupancy contributes to sustained blood pressure control
- **Once-daily dosing**: Long half-life allows once-daily administration
- **Cardiovascular protection**: Proven benefits in heart failure, post-MI, and diabetic nephropathy

## Comparison with Other ARBs

| ARB | Tetrazole | Arg167 Interactions | Lys199 Interaction | Dissociation |
|-----|-----------|-------------------|-------------------|--------------|
| Candesartan | Yes | 2 salt bridges | Yes (tetrazole) | Very slow |
| ZD7155 | Yes | Extensive network | No | Slow |
| Valsartan | No | 1 salt bridge | No | Moderate |
| Losartan | Yes | 1 salt bridge | No | Fast |
| Telmisartan | No | 2 salt bridges | Yes | Slow |

## Related Ligands

- [zd7155](//xray-mp-wiki/reagents/ligands/zd7155/) — High-affinity antagonist, precursor to candesartan
- [losartan](//xray-mp-wiki/reagents/ligands/losartan/) — First ARB, surmountable antagonist
- [valsartan](//xray-mp-wiki/reagents/ligands/valsartan/) — Clinically used ARB
- [irbesartan](//xray-mp-wiki/reagents/ligands/irbesartan/) — Clinically used ARB