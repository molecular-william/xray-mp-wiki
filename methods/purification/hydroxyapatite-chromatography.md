---
title: Hydroxyapatite Chromatography
created: 2026-05-28
updated: 2026-05-28
type: method
category: methods
layout: default
tags: [purification-ion-exchange, subdirectory-purification]
sources: [doi/10.1016##j.str.2012.11.010]
verified: false
---

# Hydroxyapatite Chromatography

## Overview

Hydroxyapatite chromatography is a separation technique that exploits the affinity of proteins for calcium phosphate crystals. It is commonly used as a polishing step in membrane protein purification, particularly for enzymes and complexes that bind to phosphate groups. Elution is typically achieved by increasing phosphate buffer concentration, which competes for binding sites on the hydroxyapatite matrix.

## Proteins Using This Method

| Protein | Resolution | PDB | Notes |
|---|---|---|---|
| [EcHyd-1](/xray-mp-wiki/proteins/ec-hyd-1/) | 3.3 A | 4GD3 | Final purification step after Superdex 200; eluted at high phosphate concentration |
